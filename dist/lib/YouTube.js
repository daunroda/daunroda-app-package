...
class YouTube {
    ...
    init() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            this.client = yield youtubei_js_1.Innertube.create({});
            return this;
        });
    }
    processSongs(processed) {
        var _a;
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            for (const playlist of processed) {
                yield (0, fs_utils_1.ensureDir)((0, path_1.join)(this.daunroda.config.downloadTo, (0, sanitize_filename_1.default)(playlist.name)));
                const promises = [];
                ...
                for (const song of playlist.songs) {
                    ...
                    if (yield (0, fs_utils_1.exists)(destination)) {
                        songs.push(name);
                    ...
                    const searched = yield this.client.music.search(name, { type: "song" });
                    const result = ((_a = searched === null || searched === void 0 ? void 0 : searched.results) === null || _a === void 0 ? void 0 : _a.length)
                        ? // Find the first result that doesn't get filtered out
                            yield searched.results.map((res) => this.filter(res, name, destination, track, playlist.name, notFound))[0]
                        : null;
                    if (!result) {
                ...
                const m3u8 = songs
                    ...
                    .join("\n");
                yield (0, promises_1.writeFile)((0, path_1.join)(this.daunroda.config.downloadTo, `${(0, sanitize_filename_1.default)(playlist.name)}.m3u8`), m3u8);
            ...
            for (const download of this.downloadMaybe) {
                if (yield (0, fs_utils_1.exists)(download.destination))
                    continue;
    ...
    downloadSong(id, destination, track, playlist, total, downloaded) {
        var _a;
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            let tmpImg = null;
            if (coverUrl) {
                const coverStream = yield (0, undici_1.request)(coverUrl).then((res) => res.body.arrayBuffer());
                tmpImg = (0, path_1.join)((0, os_1.tmpdir)(), `${(Math.random() + 1).toString(36)}.jpg`);
                yield (0, promises_1.writeFile)(tmpImg, Buffer.from(coverStream));
            }
            const tmpAudio = (0, path_1.join)((0, os_1.tmpdir)(), `${(Math.random() + 1).toString(36)}.${this.daunroda.config.audioContainer}`);
            yield this.saveTmpAudio(audioStream, tmpAudio);
            return new Promise((resolve, reject) => {
                try {
                    ...
                    ff.on("error", reject);
                    ff.on("end", () => tslib_1.__awaiter(this, void 0, void 0, function* () {
                        if (tmpImg)
                            yield (0, promises_1.rm)(tmpImg);
                        yield (0, promises_1.rm)(tmpAudio);
                        this.daunroda.emit("progress", {
                            playlist,
                            downloaded,
                            total,
    ...
    filter(res, name, destination, track, playlist, notFound) {
        var _a, _b, _c, _d, _e;
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (!notFound.has(name))
                notFound.add(name);
            // Don't download age restricted songs or if you can't fet info on something
            const info = yield (0, ytdl_core_1.getBasicInfo)(`https://youtu.be/${res.id}`).catch(() => null);
